...
class nlosExclusion():
    ...
    def nlosExclusion_(self, dataPB, dataNG, mode, manuSatLis):
        ...
        self.lateralDis = 6 # for Transaction on Vehicle Technology
        ...
        if (self.nlExcMode == 'statManu'):
            for saIdx in range(len(self.nlosGNSS_.GNSS_Raws)):
                if (self.nlosGNSS_.GNSS_Raws[saIdx].prn_satellites_index in self.mannuSat):  # if satellite is Light of sight (save)
                ...
                elif(self.nlosGNSS_.GNSS_Raws[saIdx].prn_satellites_index not in self.mannuSat):  #no correction
